    ...
    */
	...
	private boolean isParamsDiffer(final Map<String, Object> fsFileParams,
		...
		for (String key : fsFileParams.keySet()) {
			...
			if (CmsImportExportManager.N_PROPERTIES.equals(key)
					&& !isListEquals((List<CmsProperty>) fsFileParams.get(key),
							(List<CmsProperty>) vfsFileParams.get(key),
							propertyComparator)) {
				return true;
			}
			if (CmsImportExportManager.N_ACCESSCONTROL_ENTRIES.equals(key)
	...
	private void fillFileParamMap(final Map<String, Object> fileParamMap,
		...
		final Element currentElement = (Element) this.getDocXml()
				.selectNodes("//" + CmsImportExportManager.N_FILE).get(0);
		...
		fileParamMap.put(CmsImportExportManager.N_TYPE, type);

		// <destination>
		final String destination = XmlHandling.getChildElementTextValue(
				currentElement, CmsImportExportManager.N_DESTINATION);
		fileParamMap.put(CmsImportExportManager.N_DESTINATION,
		...
		fileParamMap.put(CmsImportExportManager.N_UUIDSTRUCTURE, uuidstructure);
		...
		if (!type.isFolder()) {
			...
			fileParamMap.put(CmsImportExportManager.N_UUIDRESOURCE, null);
		}
		...
		// <accesscontrol>
		fileParamMap
				.put(CmsImportExportManager.N_ACCESSCONTROL_ENTRIES,
						readParamFromVfsFile ? this.getCms()
	...
	private List<CmsAccessControlEntry> getACEList(final String uuidresource,
		...
		for (int j = 0; j < acentryNodes.size(); j++) {
			...
			final String id = XmlHandling.getChildElementTextValue(
					currentEntry, CmsImportExportManager.N_ACCESSCONTROL_PRINCIPAL);
			...
			try {
				...
				final String acflags = XmlHandling.getChildElementTextValue(
						currentEntry, CmsImportExportManager.N_FLAGS);
